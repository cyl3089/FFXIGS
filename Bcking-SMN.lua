...
function set_style(sheet)
    send_command('@input ;wait 3.0;input //lua load equipviewer '..sheet)
    send_command('@input ;wait 5.0;input /lockstyleset '..sheet)
    add_to_chat (21, 'Lockstyle Set!')
    add_to_chat (55, 'You are on '..(' S-M-N '):color(5)..''..(''):color(55)..''..('Macros set!'):color(121))
end
set_style(18)
	...
	elseif command:lower()=="meleemode" then
		if cmdParams[2] then	
			if T{'lock', 'on', 'true'}:contains(cmdParams[2]:lower()) then
				MeleeMode = true
				equip(sets.nirvanal)
				disable("main","sub","ammo","head","body","hands","legs","feet","neck","waist","left_ear","right_ear","left_ring","right_ring","back")
